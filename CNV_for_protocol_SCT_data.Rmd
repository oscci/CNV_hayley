...
preregistered script date: "06/06/2018"
CNV analysis script date: "11/11/2018 (DB)"
output:
...
# Modified from preregistered script to incorporate real CNV data 11/11/19
# N.B. relevant chunks now moved to 'script_for_AJMG.rmd.'
...
hayleydir<-'c:/Users/hayle/Dropbox/_SLI/Trisomy/SLIC data CNV/'
...
## Slot in new dataset for previously determined analysis
...
Code chunks below that were previously used with dummy data are now applied to the actual dataset.
NB. In this dataset we have included variables to indicate ascertainment bias - for the SCT cases this is the same as the high bias and low bias groups previously reported by Bishop et al: high bias coded if the trisomy was detected because of investigations for neurodevelopmental or behavioural problems, low bias otherwise.  For the twins, we distinguish those whose parents volunteered them for the study because one or both twins had language problems. 
...
#Originally read in cnvpheno, which does not have ethnicity.
#Added ethnicity in cnvpheno2, but NB this is potentially identifying so this version of file must not be made open.
#ALso added language concerns and N lang test low to cnvpheno2
cnvpheno<-read.csv('/Users/dorothybishop/deevybee_repo/CNV_Hayley/cnvpheno2.csv',stringsAsFactors=FALSE)
cnvpheno$ethnicbinary <-2 #set so all nonEuropean are 2, all Euro are 1
w<-which(cnvpheno$ethnic=='Eur')
cnvpheno$ethnicbinary[w] <- 1
cnvpheno$Group1<-cnvpheno$Group
cnvpheno$Group<-0
w<-which(cnvpheno$Group1=='twin')
cnvpheno$Group[w]<-1
...
cnvpheno$Group<-as.factor(cnvpheno$Group)
#-----------------------------------------------------------------------------#
```
...
p1 <- ggplot(cnvpheno,aes(x=sum_pLi_C,y=log(total_Kbp),colour=factor(Group))) + geom_point() +
  scale_x_continuous(expand=c(0.02,0)) +
  scale_y_continuous(expand=c(0.02,0)) +
  theme_bw() +
  theme(legend.position="none",plot.margin=unit(c(1,1,1,1),"points"))
...
p2 <- ggplot(cnvpheno,aes(x=sum_pLi_C,colour=factor(Group),fill=factor(Group))) + 
  geom_density(alpha=0.5) + 
  scale_x_continuous(breaks=NULL,expand=c(0.02,0)) +
  scale_y_continuous(breaks=NULL,expand=c(0.00,0)) +
  theme_bw() +
...
p3 <- ggplot(cnvpheno,aes(x=total_Kbp,colour=factor(Group),fill=factor(Group))) + 
  geom_density(alpha=0.5) + 
  coord_flip()  + 
  scale_x_continuous(labels = NULL,breaks=NULL,expand=c(0.02,0)) +
  scale_y_continuous(labels = NULL,breaks=NULL,expand=c(0.00,0)) +
...
mycorr<-cor(cnvpheno$sum_pLi,cnvpheno$total_Kbp)


##############################################################################
```
This dataset has a group variable indicating type of trisomy or twin. We see how the different CNV burden indicators compare. 
In our original test dataset, burden indicators were correlated. Here the correlation is `r mycorr`
...
#print('Total CNVs')
w1<-wilcox.test(total_cnvs~Group,data=cnvpheno,alternative=c("less"),conf.int=TRUE)
#print(list(p.value=w1$p.value,corrected.alpha=BF_correct_alpha,res=w1$p.value<=BF_correct_alpha))
#print('Total Kbp')
w2<-wilcox.test(total_Kbp~Group,data=cnvpheno,alternative=c("less"),conf.int=TRUE)
#print(list(p.value=w2$p.value,corrected.alpha=BF_correct_alpha,res=w2$p.value<=BF_correct_alpha))
#print('sum pLi')
w3<-wilcox.test(sum_pLi_C~Group,data=cnvpheno,alternative=c("less"),conf.int=TRUE)
...
```{r poisson, echo=TRUE}
#-----------------------------------------------------------------------------#
summary(m1 <- glm(global_neurodev ~ sum_pLi_C + Group1 + sum_pLi_C:Group1, family="poisson", data=cnvpheno))
...
for (pheno in 1:2){
summary(m1 <- glm(langfactor ~ sum_pLi_C + Group1 + sum_pLi_C:Group1, family="gaussian", data=cnvpheno))
if(pheno==2){
  summary(m1 <- glm(piq ~ sum_pLi_C + Group1 + sum_pLi_C:Group1, family="gaussian", data=cnvpheno))
}
#check the assumption that mean and variance ar roughly equal. If not, we can use robust estimates. I think this will make little difference in our case but included for completeness.
