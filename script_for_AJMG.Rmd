...
makesyn <- 1 #set to 1 to create a synthetic dataset (see end of chunk)
...
#now create synthetic dataset to preserve confidentiality
  ...
  for (t in 0:1){
    for (b in 0:1){
      ...
      w <-intersect(w1,w2)
      tempsyn<-syn(cnvpheno[w,c(42,44:52,54:60,62:64)])$syn #create synthetic dataset
      if (r==1){
        mysyn<-tempsyn
      }
      if (r>1){
...
write.csv(mysyn,'cnv_syn.csv',row.names=FALSE)
}
